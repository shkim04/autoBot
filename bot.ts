...
const USD_TERA_SYMBOL = 'USDT';

const COIN_SET = ['BTC'];
const btcSymbol = `BTC${USD_TERA_SYMBOL}`;
...
async function bot() {
  try {
    const btcData = await client.prices({ symbol: btcSymbol });
    const btcPrice = btcData[btcSymbol];

    const btcCandles = await client.candles({ symbol: btcSymbol, interval: '2h', limit: 50 });

    for (let i = 0; i < btcCandles.length; i++) {
      btcRsi.update(btcCandles[i].close);
    }

    const latestRsi = parseFloat(btcRsi.getResult().toFixed(2));
    const accountInfoData = await client.accountInfo();
    const btcBalance = accountInfoData.balances.find(b => b.asset === 'BTC')?.free || '0';
    const usdtBalance = accountInfoData.balances.find(b => b.asset === 'USDT')?.free || '0';
    const sellQty = parseFloat(btcPrice) * parseFloat(btcBalance);
    const inPosition = btcBalance !== '0' ? true : false;

    const isStillUpTrend =
      previousRsi.length > 5 &&
    ...
    if (inPosition) {
      if (sellCheck(latestRsi)) {
        const sellResponse = await client.order({
      ...
      if (buyCheck(latestRsi)) {
        const buyResponse = await client.order({
          type: OrderType.MARKET,
          symbol: btcSymbol,
          side: 'BUY',
